...
export function AppShell() {
    ...
    ...(isOperatorNav
      ? [
          ...
          ...(showCitizenDashboard
            ? [{ pageKey: 'dashboard' as const, path: '/dashboard', label: t('nav.citizenPanel', 'Vatandaş Paneli'), icon: LayoutDashboard }]
            : []),
      ...
      : isReporterNav
        ? [
            ...
            ...(isInternalModuleUsable
              ? [
                  { pageKey: 'dashboard' as const, path: '/dashboard/birimler', label: t('nav.dashboardDepartments', 'Anasayfa - Birimler'), icon: LayoutDashboard, separatorBefore: true },
      ...
      if (item.children && item.children.length > 0) {
        ...
        items.push({
          ...
          children: visibleChildren.map(child => ({
            ...
            multilineLabel: child.multilineLabel,
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[color:var(--color-sidebar)] lg:h-dvh lg:overflow-hidden">
      ...
      <div style={{ zoom: zoom.content }} className="app-content-shell m-0 min-w-0 flex-1 bg-white shadow-sm sm:my-2 sm:mr-2 sm:rounded-2xl sm:border sm:border-[var(--color-border)] md:flex md:min-h-0 md:flex-col md:overflow-visible">
        <div className="relative z-40 hidden items-center justify-between overflow-visible border-b border-[var(--color-border)] bg-white/94 px-5 py-2.5 backdrop-blur lg:flex">
          ...
          <div className="flex items-center gap-3 overflow-visible pt-0.5">
            <ExtensionSearchBar />
            <GlobalSearchBar />
        ...
        <main
          id="main-content"
          className={`flex min-h-0 w-full max-w-none flex-1 flex-col overflow-x-hidden overflow-y-auto px-3 py-3 sm:px-4 lg:px-5 lg:py-3 xl:px-6 2xl:px-7${location.pathname === '/whatsapp' ? ' md:overflow-y-hidden' : ''}`}
        >
