  ...
  return createPortal(
    <ModalBackdrop>
      ...
      >
        ...
        {state.title && (
          <h2
            className={`text-slate-950 ${state.titleCompact ? 'text-base font-semibold' : 'text-lg font-bold'} ${state.titleUnderline ? 'underline underline-offset-4' : ''} ${state.titleDivider ? 'mb-3 border-b border-slate-200 pb-2' : 'mb-2'}`}
          >
            {state.title}
          </h2>
        )}
