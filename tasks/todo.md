...
- [ ] `#610` `#637` `#585` `#638` — ERTELENDİ (kullanıcı kararı: "Stop here for now"). Hepsi tarayıcı zoom'u (%80↔%90) / ekran boyutuna (15.6") özgü CSS sorunları; bu ortamda reprodüksiyon/doğrulama yapılamıyor: (1) seed yok → rol-kısıtlı Görevlerim alt görünümlerine (Birimdeki/Personelimin) girilemiyor; (2) `preview_resize` gerçek tarayıcı zoom metriklerini (outerWidth/innerWidth/availWidth ayrışması — `useResponsiveZoom`'un dayandığı) taklit edemiyor; (3) #610 önceki turda da reprodüksiyon edilememişti, üç görünümün tablo/çip yapısı birebir aynı → yapısal neden yok, zoom/sticky render artefaktı. `main` prod'a otomatik deploy ettiğinden kör/doğrulanamayan layout değişikliği tüm kullanıcılarda regresyon riski. Doing'de bırakıldı; cihaz metrikleri (OS ölçek %, %80 ve %90'da availWidth/innerWidth/outerWidth) veya canlı pair gerekiyor.
