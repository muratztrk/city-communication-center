...
- **Görev popup metin limitleri (#3400/#3401):** Durum Değişikliği nedeni, Tamamlama Notu ve
...
- **Completed/Cancelled görev yeniden tamamlanamaz** (`CompleteTaskCommand` guard). Durumu
  ...
  görünür; "Yazışmaya Git" varsa solunda kalır.
  Görevlerim / Birimdeki Görevler / Personelimin Görevleri detay popup'ında **Düzenle** yok (#3587).
...
- **Adres girişleri mahalle kapılıdır:** talep/rutin/e-Devlet/Taleplerim düzenleme formlarında
  ...
  Detaylar → Düzenle'de Mahalle / Cadde / No CBS dropdown'dır; Adres Tarifi metin kalır (#3586).
...
- **WhatsApp konuşma footer aksiyonları:** Alt aksiyon satırında `Talep oluştur` butonu görünmez;
  ...
  Vatandaş Çağrı Talebi oluşturma popup'ında Şablon mesajlar/Şablon mesaj ekle/Dosya ekle
  kontrolleri yalnız o popup'a özel kompakt 28px. Genel dropdown ölçüleri etkilenmez.
...
- **Mesajı İncelemeye Gönder (#6aad490e):** operatör `/whatsapp`'ta talep olan birimlere dropdown ile gönderir;
  ...
  masaüstünde (`min-width: 1024px`) Dosya ekle ile aynı satırda, hemen yanında; mobilde şablon/Dosya ekle satırına çıkmaz, kendi satırında kalır (#6ab26b77)
  ve kapalı kontrol ile açılan liste, alttaki mesaj kutusunun genişliğindedir (`min-width: 100%`, gönder düğmesi tekrar kırpılmaz, #6ab362d3);
  ...
  Mobilde (`max-width: 1023px`) açılan inceleme paneli tetikleyiciyle aynı genişliktedir; +48px yalnız masaüstü, sağa (#6ab19450, #6ab23335).
...
- **WhatsApp konuşma scroll'u kullanıcı niyetini korur:** `/whatsapp` detayında kullanıcı mesaj alanında
  ...
  Kutucuk değişince gri pulse iskelet (`ConversationSkeleton`) gösterilmez — o “görünüp kaybolan
...
- **İptal/Tamamlanma terminal notu durum mesajına gömülür (card #2103, #1829 supersede):**
  ...
  `{VT-no} no'lu talebinizin eki gönderilmiştir.` + `[Dosya eki: dosya]`; eski yalnız-işaret
  ve kısa `… eki` / noktasız `… gönderilmiştir` kayıtlar okumada ve operatör gönderiminde yeni metne yükseltilir. Popup/terminal not metadata (Not chip) korunur.
...
- **Görevlerim İlgili Talep Adres Tarifi (#2778 / #3327 reopen):** Görevlerim/Birimdeki/Personelimin popup’ta
  `Adres Bilgileri` eşit 3 kolon sola yaslı (`repeat(3, 1fr)`); Mahalle/Cadde/No satır 1,
  Adres Tarifi/Konum Linki satır 2; satır 2 üst boşluk `#2651` korunur.
...
- **Görev Bilgileri üst metası ve alan sırası:** Görevlerim/Birimdeki Görevler/Personelimin Görevleri
  ...
  **Sistemde ara…** sağda; her ikisi de en az 3 karakterde arar. Personel araması yalnız
  ...
  Header Personel/Sistemde ara placeholder `text-xs` + `placeholder:font-light` (#6a926f4e / #6a92f91b).
  ...
  Sistemde ara kanal ikonu vatandaş kaynağını `isCitizenRequestJob` (requestType **veya**
  sourceType) ile bulur — `requestType === 'Citizen'` şartı yetmez, WhatsApp talebi
  `ExternalUnit` da olabilir. Sosyal liste yetkiden dolayı boş gelirse kanal `sourceType`’tan
  türetilir (`SocialMessage`→WhatsApp, `EDevlet`→EDevlet, diğer→Phone); ikon **null olmamalı**.
  İkon başlık `span`’ının içinde (ayrı kolonda `self-center` iki satırın ortasına kaçıyordu).
...
- **Birimler/Kullanıcılar grid:** FilterableTh + sort + TablePagination; kolon genişlikleri
  ...
  **Aktif** Rol kolonunun altında (Ek roller satırını itmez) (card #1718). LDAP kullanıcı
  ...
  şart değil); dropdown her zaman görünür. Genişlik `12–16rem`. Placeholder **span** `0.68rem`
  (admin-surface `.field-select span` 0.95rem’i ezer, #3307). Açık liste satırı `min-height` 1.85rem (#3313).
...
- **Dosya sunucusu test alanları (#6a6cb6ec):** NAS ve FTP kolonlarında ayrı bağlantı +
  ...
  (`********`) veya boş alan eski parolayı korur. Tarayıcı otomatik doldursa da kutudaki değer
  esas alınır. NFS’te klasör yazılmaz; eksik kimlik bilgisi kaydı Türkçe hata döner.
...
- **NAS'ta silinmiş ek (#3398):** İndirme/Ön İzle 404 dönerse talep/görev ayrımına göre popup:
  "Talep eki sistemde bulunamadı…" / "Görev eki sistemde bulunamadı…"; başlık altında ayırıcı çizgi.
...
- **Vatandaş Bilgi Listesi (card #1836, kolon/buton düzeni #1843/#1858):** `/citizen-directory` yalnız
  ...
  (#VT-2026-99; "Saygılarımızla" kuyruğu Vatandaşa Giden Mesaj olmaz). Çağrı/SMS'te
  `ResponseContent` son ara durum (Yapılmakta) mesajıysa aynı kural: outbound boş kalır,
  Tamamlama Notu görev/onay notudur, Vatandaşa Giden Mesaj mavi `Onay Bekleyen` (VT-2026-136). Release sonrası terminal
...
- **Atanmış görev adres stacked (#r496/#r497/#6a6baec9 / #2651 / #3327 reopen):** Görevlerim İlgili Talep Adres
  `variant="my-request"` (Mahalle+Cadde+No / Adres Tarifi+Konum); eşit 3 kolon sola yaslı.
...
- **Ayarlar Lisans sekmesi:** lumespec-license'tan (Ed25519 imzalı) gerçek modül durumu gösterilir.
  ...
  **Kurum İçi kapalıyken (#2369/#2376/#2377/#2378):** `outgoingRequests` menü/arama/dashboard'dan gizlenir
  (`PAGE_LICENSE_MODULE` → `internal`); `createRequest` yalnızca Operator + citizen lisans açıkken;
...
- **Whatsapp Mesaj Logları:** sol menüde İade Edilen Talepler altında. Yanıt Verildi ve
  ...
  Mesaj Onayı/Cevabı Verildi turuncu. İşlem Tarihi takvim ikonlu, saniyesiz, sütun diğer tarih
  hücrelerinden biraz geniş. Seçili filtre bannerın ilk satırında. Başlık diğer gridlerle aynı yeşil.
...
- **Vatandaş Talepleri mobil (`max-width: 1023px`):** Yönetici Onayı Bekleyen etiketi sola, tam satır.
  Mobil Uygulama çipi üst satırdaki e-Devlet/Çağrı ile aynı boy. Tüm Talep Durumları onlardan biraz küçük.
...
- **Mesaj Onayı Giden/Tümü grid (#3530):** `sent`/`all` scope'ta `Durum` yerine `Mesaj Onayı Yapan`
  ...
  `Onay Bekleyen` (`text-sky-500 text-[0.875rem]`, #3564).
...
- **Boş adres `-` her görünümde başlığın SOL kenarında (#3143):** `-` ortalanmaz (önceki tur
  ...
  Ortalı kolonlarda (`--coordinates` genel) `-`'nin başlığın
  ...
  (`--task` #3327, `--empty` üç kolon #6a6dab1b, `--attachments-only` #3327) ile
