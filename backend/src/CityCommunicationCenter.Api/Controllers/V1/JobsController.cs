...
public sealed class JobsController : ApiControllerBase
{
    ...
    [HttpPost("{jobId:guid}/forward-returned")]
    public async Task<IActionResult> ForwardReturned(
        Guid jobId,
        [FromBody] ForwardReturnedCitizenRequestRequest request,
        CancellationToken cancellationToken)
    {
        var ok = await _sender.Send(
            new ForwardReturnedCitizenRequestCommand(jobId, request.TargetDepartmentId, CurrentContext.UserId, request.Note),
            cancellationToken);
        return ok ? NoContent() : NotFound();
    }
    ...
    {
        var ok = await _sender.Send(
            new FillJobCbsAddressFromCoordinatesCommand(jobId, CurrentContext.UserId, districtId ?? string.Empty, streetNo),
            cancellationToken);
        return ok ? NoContent() : NotFound();
    }
