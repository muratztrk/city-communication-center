...
function formatDisplay(value: string, dateOnly = false): string {
  ...
  // Tarih ile saat arasında daha geniş boşluk (#r545 / #9 reopen).
  return `${base}\u00A0\u00A0\u00A0${h}:${m}`
}
        ...
        >
          ...
          <div className="flex items-center gap-2 border-t border-slate-100 px-3 py-2.5">
            ...
            <button
              ...
              disabled={!draft.date}
              className="date-time-picker-select shrink-0 rounded-lg bg-[color:var(--color-primary)] px-4 py-1.5 text-sm font-bold text-white shadow-sm transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Seç
            </button>
          </div>
