...
- [~] `#621` — "WhatsApp konuşmaları gridview'de satır olmasın." Zaten `1e36cc9` "Hide WhatsApp conversations from citizen requests" ile uygulanmış (SocialMessagesPage `message.channel !== 'WhatsApp'` + WhatsApp çipi kaldırıldı). HEAD = origin/main = deploy. Kod isteneni yapıyor → muhtemelen stale PWA cache.
- [~] `#634` — Talep/görev süreçlerindeki tüm değişiklikler ilgili tüm kullanıcıların bildirimlerinde görünmeli. Mevcut mimari zaten kapsıyor: bildirim feed'i AuditLog'lardan türetiliyor (GetNotificationsQuery, tüm action tipleri Türkçe başlıklarla) ve NotificationAudience oluşturan/atanan/sahip + sahip/hedef/koordine birim yöneticisini (card 541) dahil ediyor. Listelenen senaryolar (gelen talep, onay, personel atama, oluşturma/değiştirme/iptal) kapsanıyor görünüyor. Belirli eksik bir senaryo varsa netleştirme bekleniyor; aksi halde stale cache.
- Stale cache kök neden: PWA `registerType:'autoUpdate'`+`skipWaiting`+`clientsClaim` agresif ama açık sekme yeniden yüklenene kadar eski bundle'ı tutuyor; #612/#610/#621/#631 hep deploy'da mevcut ama kullanıcı eski sürümü görüyor.
- FE build + lint PASS.
