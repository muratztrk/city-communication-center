...
function NotificationTitleStatusText({
  ...
  const emphasis = isUnread ? 'font-semibold' : 'font-medium'
...
function NotificationTitle({ title, isUnread }: { title: string; isUnread: boolean }) {
  const mainWeight = isUnread ? 'font-semibold text-slate-900' : 'font-medium text-slate-700'
  ...
  return (
    <>
      {tone ? (
        <span className={`${isUnread ? 'font-semibold' : 'font-medium'} ${tone}`}>{mainText}</span>
      ) : (
