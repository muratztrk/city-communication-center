...
// 2. Admin switches to LDAP mode, reads the explicit onboarding copy, and links an unclaimed directory user.
...
test('admin can create manual and LDAP users from the users screen', async ({ page, request }) => {
  ...
  await test.step('Admin creates a manual user', async () => {
    await page.getByRole('button', { name: /yeni kullanici/i }).click();
    await page.getByLabel('Kullanici Adi').fill(manualUsername);
  ...
  await test.step('Admin creates an LDAP-linked user from directory search', async () => {
    await page.getByRole('button', { name: /yeni kullanici/i }).click();
    await page.getByRole('button', { name: 'LDAP Kullanicisi', exact: true }).click();
    await expect(page.getByText(/otomatik olusturulmaz/i)).toBeVisible();
    await selectAutocompleteOption(page, 'Dizin kullanicisi ara', directoryCandidate!.displayName);
    await page.getByLabel('Departman').selectOption(COMMUNICATIONS_DEPARTMENT_ID);
    await page.getByLabel('Rol').selectOption({ label: 'Raporlayici' });
    await page.locator('button[type="submit"]').last().click();
