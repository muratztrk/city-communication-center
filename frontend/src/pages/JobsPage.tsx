...
function buildCancelledWithoutTaskInfoRows(
  detail: JobDetail,
  t: TFunction,
...
function getJobGridStatusPillClass(
  ...
  if (isCitizenRequestJob(job)) {
    return getStatusPillClass(getCitizenRequestStatusTone(job))
  }
...
export function JobsPage({ fixedScope, mode = 'external', notificationJobId, detailOnly = false, detailContextOverride, onNotificationDetailClose, onChangeStatusToInProgress, messageApprovalActions, hideMessageApprovalPendingFields = false, showRequestInfoCitizenOutbound = false, socialActions }: JobsPageProps) {
  ...
  const jobsTableColumnCount = useMemo(() => {
    ...
    if (showDepartmentOutgoingCreatedBy) count += 1
    count += 1
    ...
    if (showJobsGridStatusColumn) count += 1
    count += 1
    return count
  ...
  const renderOutgoingDestination = (job: JobSummary) => {
    ...
    return (
      <div className="mx-auto flex min-w-[11rem] max-w-[22rem] flex-col items-center gap-1">
        <div className="flex flex-wrap justify-center gap-1.5">
          ...
          {hiddenTargetCount > 0 ? (
            <StatusPill tone="neutral" className="px-2.5 py-1 text-[0.76rem]">{t('jobs.departmentsMore', { count: hiddenTargetCount, defaultValue: '+{{count}} müdürlük' })}</StatusPill>
          ) : null}
        </div>
      ...
      <header className="sticky-page-header">
        <div className="page-header-row">
          <div className="space-y-1">
            {/* Banner ilk satır = seçili sekme metni (card #1700; Taleplerim/Gelen ile aynı kalıp). */}
            ...
            <table className={`data-table jobs-table${isMyRequestsView || isDepartmentOutgoingView ? ' my-requests-table' : ''}${isMyRequestsView || isDepartmentOutgoingView ? ' data-table--zebra' : ''}`}>
              {(isMyRequestsView || isDepartmentOutgoingView) && (
                <colgroup>
                  ...
                  {showDepartmentOutgoingCreatedBy && <col className="grid-col-created-by" />}
                  ...
                  {!destinationBeforeOwner && <col className="grid-col-destination" />}
                  {showJobsGridStatusColumn && <col className="grid-col-status" />}
              ...
              <thead>
                <tr>
                  ...
                  {showDepartmentOutgoingCreatedBy && <FilterableTh filterKey="createdByDisplayName" filterValue={jobFilters['createdByDisplayName'] ?? ''} onFilter={setJobFilter} sortKey="createdByDisplayName" currentSortKey={jobsSortKey} sortDir={jobsSortDir} onSort={toggleJobsSort}>{t('jobs.columns.createdBy', 'Oluşturan')}</FilterableTh>}
                  ...
                  {showJobsGridStatusColumn && <FilterableTh filterKey="status" filterValue={jobFilters['status'] ?? ''} onFilter={setJobFilter} sortKey="statusSortText" currentSortKey={jobsSortKey} sortDir={jobsSortDir} onSort={toggleJobsSort}>{t('jobs.columns.status', 'Durum')}</FilterableTh>}
              ...
              <tbody>
                ...
                {pagedJobs.map((job, index) => {
                  ...
                  <tr key={job.jobId}>
                    ...
                    {showDepartmentOutgoingCreatedBy && <td><EmptyCell value={job.createdByDisplayName} /></td>}
                    ...
                    {showJobsGridStatusColumn && (() => {
                      ...
                      return (
                        <td>
                          ...
                          {activeJobView === 'all' && isTerminalJob && jobExtraTimeMarkers}
                        </td>
                      )
                    })()}
                  ...
                  <div className="min-w-0 p-4">
                    {(() => {
                      ...
                      const processSteps = buildJobProcessSteps(t, detail, locale, {
                        ...
                        // Gelen ve Giden birim dışı: hedef onayı beklerken tek sentetik adım (cards #1641/#1642).
                        showPendingTargetApprovalAfterStatus: true,
