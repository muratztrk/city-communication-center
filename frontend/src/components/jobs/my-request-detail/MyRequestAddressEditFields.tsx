...
export function MyRequestAddressEditFields({ draft, onChange, operatorSocialLayout = false }: MyRequestAddressEditFieldsProps) {
  ...
  const openAddressField = (
    <label className="grid min-w-0 gap-1">
      <span className={`text-xs font-semibold text-slate-500${operatorSocialLayout ? ' flex min-h-[3.5rem] items-end gap-1.5' : ''}`}>
      ...
      <textarea
        className={`field-textarea resize-none disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400${operatorSocialLayout ? ' min-h-[5.5rem] placeholder:text-[0.52rem]' : ' min-h-[2.75rem]'}`}
