import { Check, FileText, PenLine, Search, X } from 'lucide-react'
...
function TerminalCitizenMessageApprovalPage({ mode }: { mode: ApprovalChannelMode }) {
  ...
  const getColumnValue = useCallback((key: string, row: CitizenMessageApprovalRow): string => {
    ...
    if (key === 'messageApproverDisplayName') {
      const name = row.messageApproverDisplayName?.trim()
        || t('citizenMessageApproval.pendingApprover', 'Onay Bekleyen')
      const date = row.messageApprovedAtUtc ? formatDateTime(row.messageApprovedAtUtc, locale) : ''
                ...
                <tr>
                  ...
                  <FilterableTh className="citizen-message-approval-citizen-col" filterKey="citizenName" filterValue={filters['citizenName'] ?? ''} onFilter={setFilter} sortKey="citizenName" currentSortKey={sortKey} sortDir={sortDir} onSort={toggleSort}>
                    <span className="inline-flex flex-col gap-1 leading-tight">
                      ...
                      <span className="text-[0.9em] font-bold leading-tight">{t('citizenMessageApproval.columns.citizenPhone', 'Telefon No')}</span>
                    </span>
                  </FilterableTh>
                ...
                {paginatedRows.map((row, index) => (
                  <tr key={row.jobId}>
                    ...
                    <td className="citizen-message-approval-citizen-cell">
                      ...
                      <div className="citizen-message-approval-phone-value font-medium text-slate-600">{formatCitizenPhoneDisplay(row.citizenPhone)}</div>
                    </td>
